...
export class DjService {
  ...
  async start(userId: string, preferences?: { mood?: string; genres?: string[] }) {
    ...
    const firstTrack = await this.getNextTrackFromAI(userId, session, recentTracks);

    return { sessionId: userId, status: 'active', firstTrack, mood: session.mood };
  }

  async next(userId: string) {
    const session = await this.redis.getJson<DjSession>(`dj:session:${userId}`);
    if (!session) return { error: 'No active DJ session', message: 'Start a DJ session first' };
  ...
  async command(userId: string, text: string) {
    ...
    if (!this.ai.isConfigured()) {
      return { message: 'Command received', interpreted: text, action: 'ai-not-configured' };
    }

    const response = await this.ai.chatJson<{ action: string; mood?: string; energy?: string; genre?: string; response: string }>([
      {
        role: 'system',
        content: 'You are an AI DJ assistant. Interpret user commands about music. Return JSON: {"action": "change_mood|change_energy|change_genre|skip|replay|info", "mood": "new_mood_if_changed", "energy": "low|medium|high", "genre": "genre_if_changed", "response": "DJ response text"}',
      },
    ...
    if (response.genre) session.genres = [response.genre, ...session.genres.slice(0, 4)];
    await this.redis.setJson(`dj:session:${userId}`, session, 4 * 3600);

    return { ...response, sessionUpdated: true };
  }

  async feedback(userId: string, data: { trackId: string; type: 'up' | 'down' }) {
    const session = await this.redis.getJson<DjSession>(`dj:session:${userId}`);
    if (!session) return { error: 'No active DJ session' };
    ...
    return { message: 'Feedback recorded', totalFeedback: session.feedback.length };
  }
  ...
  async commentary(userId: string, trackId: string) {
    ...
    const track = await this.prisma.track.findUnique({ where: { youtubeId: trackId } });
    const trackName = track ? `${track.title} - ${track.artist || 'Unknown'}` : trackId;

    const response = await this.ai.chatJson<{ commentary: string; funFact: string; transition: string }>([
      {
        role: 'system',
        content: 'You are an enthusiastic AI DJ. Write a short, engaging DJ commentary introducing a song. Include a fun fact. Return JSON: {"commentary": "DJ intro text (2-3 sentences)", "funFact": "interesting fact about song/artist", "transition": "transition phrase to next song"}',
      },
  ...
  private async getNextTrackFromAI(userId: string, session: DjSession, recentTracks: string[]) {
    if (!this.ai.isConfigured()) {
      return { title: 'AI DJ requires API key', artist: '', youtubeQuery: '' };
    }

    const response = await this.ai.chatJson<{ title: string; artist: string; youtubeQuery: string; djNote: string }>([
      {
        role: 'system',
