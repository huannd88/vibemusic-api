...
export class RecommendationService {
  ...
  async getForYou(userId: string, limit: number = 20) {
    ...
    if (history.length === 0) {
      ...
      return {
        tracks: trending.map((t) => t.track),
        source: 'trending-fallback',
      };
    }

    const trackTitles = history
      .map((h) => `${h.track.title} - ${h.track.artist || 'Unknown'}`)
      .slice(0, 20);
    const taste = await this.prisma.userTasteProfile.findUnique({
      where: { userId },
    });
    ...
    if (!this.ai.isConfigured()) {
      ...
      return {
        tracks: history.map((h) => h.track).slice(0, limit),
  ...
  async getRadio(userId: string, seed: string, type: string = 'track') {
    ...
    const response = await this.ai.chatJson<{
      ...
      {
        role: 'user',
        content: `Seed: ${seed} (type: ${type}). Generate 30 tracks for radio mode.`,
      },
    ]);
  ...
  async getDiscoverWeekly(userId: string) {
    ...
    if (history.length < 5 || !this.ai.isConfigured()) {
      return {
        tracks: [],
        source:
          history.length < 5 ? 'insufficient-history' : 'ai-not-configured',
      };
    }
  ...
  async getSimilar(youtubeId: string) {
    ...
    const response = await this.ai.chatJson<{
    ...
    }>([
      {
        role: 'system',
        content:
          'Given a song, find 15 similar tracks. Consider genre, mood, tempo, era, and vibe. Return JSON: {"tracks": [{"title": "...", "artist": "...", "youtubeQuery": "title artist"}]}',
      },
  ...
  async getBecauseYouListened(userId: string) {
    ...
    if (history.length === 0 || !this.ai.isConfigured()) {
      return {
        sections: [],
        source: history.length === 0 ? 'no-history' : 'ai-not-configured',
      };
    }
    ...
    const response = await this.ai.chatJson<{
    ...
    }>([
      {
        role: 'system',
        content:
          'Create "Because you listened to..." recommendations. For each of the given tracks, suggest 5 related tracks. Return JSON: {"sections": [{"becauseOf": "Song - Artist", "tracks": [{"title": "...", "artist": "...", "youtubeQuery": "title artist"}]}]}',
      },
      {
        role: 'user',
        content: `Create recommendations based on:\n${recentTracks.join('\n')}`,
      },
    ]);
  ...
  async submitFeedback(
    userId: string,
    data: { trackId: string; type: 'like' | 'dislike'; context?: string },
  ) {
    // Store feedback for taste model updates
    await this.redis.setJson(
      `ai:feedback:${userId}:${data.trackId}`,
      data,
      30 * 24 * 3600,
    );
