...
export class HumService {
  ...
  async recognize(
    userId: string,
    data: { audio?: string; description?: string },
  ) {
    ...
    if (data.description && this.ai.isConfigured()) {
      ...
      const response = await this.ai.chatJson<{
      ...
      }>([
        {
          role: 'system',
          content:
            'User is humming or describing a song melody. Identify the song. Return JSON: {"matches": [{"title": "...", "artist": "...", "confidence": 0.0-1.0, "youtubeQuery": "title artist"}]} with up to 5 matches sorted by confidence.',
        },
        {
          role: 'user',
          content: `I'm trying to find a song: ${data.description}`,
        },
      ]);
    ...
    await this.redis.setJson(
      `hum:result:${resultId}`,
      {
        ...
        status: 'pending',
        message:
          'Audio hum recognition requires integration with Google Hum-to-Search API or ACRCloud.',
        createdAt: new Date().toISOString(),
      },
      3600,
    );
    ...
    return {
      ...
      status: 'pending',
      message:
        'Audio recognition requires external API. Use description field as text fallback.',
    };
  }
